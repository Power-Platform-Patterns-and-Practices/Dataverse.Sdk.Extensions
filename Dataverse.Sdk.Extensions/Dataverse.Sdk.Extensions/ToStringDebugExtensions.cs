...
{
    public static class ToStringDebugExtensions
    {
        ...
        internal static List<string> ToStringDebug(this IExecutionContext context)
        {
            ...
            var lines = new List<string>
            {
                ...
                context.InputParameters.ToStringDebug("InputParameters", info),
                context.OutputParameters.ToStringDebug("OutputParameters", info),
                context.PostEntityImages.ToStringDebug("PostEntityImages", info),
                context.PreEntityImages.ToStringDebug("PreEntityImages", info),
                context.SharedVariables.ToStringDebug("SharedVariables", info)
            };
        ...
        {
            const string space = " "; // This is not a space, it is a Non-Breaking Space (alt+255).  In the log spaces get trimmed, and this will prevent that from happening;
        ...
        private static string GetConfigValueMaskingPasswords(string key)
        {
            ...
            if (!string.IsNullOrWhiteSpace(value) && key.IndexOf("password", StringComparison.InvariantCultureIgnoreCase) >= 0)
            {
    ...
    public class StringDebugInfo
    {
        ...
        private StringDebugInfo(StringDebugInfo toCopy, int? indentSpaces = null, int? tabWidth = null, bool? singleLine = null)
        {
            ...
            Indent = ToStringDebugExtensions.GenerateNonBreakingSpace(IndentSpaces);
            Tab = ToStringDebugExtensions.GenerateNonBreakingSpace(TabWidth);
